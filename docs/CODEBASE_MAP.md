...
> Auto-generated by Cartographer. Last mapped: 2026-04-11. This file is historical and may be stale; for current work start with `docs/NEXT_SESSION_HANDOFF.md`, `AGENTS.md`, and `README.md`.

## System Overview

Current active direction: economics-first, non-HFT Pacifica paper trading from the full-fidelity live archive. The old 25-symbol representation-learning program described in much of this generated map is historical context, not the fresh-session starting point.

**Current status:** use `scripts/collect_pacifica_full_fidelity.py` to collect all live public Pacifica symbols dynamically from `/info`, build silver with `scripts/build_pacifica_full_fidelity_silver.py`, build 1-minute regime state with `scripts/build_non_hft_regime_state.py`, and treat `scripts/non_hft_toxic_overlay_probe.py` outputs as diagnostics until enough days accrue.

**Note:** deeper directory details below were generated before the full-fidelity paper-trading pivot and should be validated against git before use.
...
├── README.md                          # Project overview; current active framing is full-fidelity paper trading
...
**To understand the project**: Read `docs/NEXT_SESSION_HANDOFF.md`, then `AGENTS.md`, then `docs/AGENT_OPERATING_MAP.md`
...
**To start implementation**: Use Hermes from the repo root; use Claude Code only if explicitly requested
**To sync data**: `rclone sync r2:pacifica-trading-data ./data/ --transfers 32 --checkers 64 --size-only`
**To run experiments**: Use current Hermes skills/tools and the active docs; repo-local Claude skills are historical unless rewritten
