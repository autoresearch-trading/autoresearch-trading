...
class PaperTradingMonitor:
    ...
    def create_stats_table(self, stats: Dict) -> Table:
        ...
        table = Table(title="Performance Stats (24h)", show_header=True, header_style="bold magenta")
        ...
        table.add_row("Best Trade", f"[green]${stats.get('best_trade', 0.0):.2f}[/green]")
    ...
    def create_trades_table(self, trades: List[Dict], limit: int = 10) -> Table:
        ...
        table = Table(title=f"Recent Trades (Last {limit})", show_header=True, header_style="bold blue")
    ...
    def run(self, refresh_seconds: int = 5, hours: int = 24):
        ...
        self.console.print(f"Refresh interval: {refresh_seconds}s | Time window: {hours}h")
        self.console.print("Press Ctrl+C to exit\n")
        
        try:
            with Live(self.generate_dashboard(hours), refresh_per_second=1, console=self.console) as live:
                while True:
