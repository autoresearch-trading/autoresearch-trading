...
class Settings(BaseSettings):
    ...
    cvd_divergence_threshold: float = Field(
        default=0.15, alias="CVD_DIVERGENCE_THRESHOLD"
    )
    ...
    ofi_signal_threshold_sigma: float = Field(
        default=2.0, alias="OFI_SIGNAL_THRESHOLD_SIGMA"
    )

    atr_period: int = Field(default=14, alias="ATR_PERIOD")
    atr_threshold_multiplier: float = Field(
        default=1.5, alias="ATR_THRESHOLD_MULTIPLIER"
    )
    ...
    extreme_funding_threshold: float = Field(
        default=0.001, alias="EXTREME_FUNDING_THRESHOLD"
    )
        ...
        if isinstance(value, str):
            ...
            if text.startswith("[") and text.endswith("]"):
                try:
                    ...
                    return [
                        str(sym).strip().upper() for sym in parsed if str(sym).strip()
                    ]
                except Exception:
                    pass
