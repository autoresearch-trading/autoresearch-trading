...
- bf16 mixed precision (`torch.autocast(dtype=bfloat16)`) + `torch.compile(encoder, mode="reduce-overhead")` **CUDA-only** — both auto-disable on MPS/CPU (MPS runs fp32, no kernel fusion). Wall-clock penalty on M-series is ~1.5× vs bf16-compile, still within the 24h cap at batch 256.
- Compute cap: ~24h wall-clock, hardware-agnostic. Measured targets: H100 at batch 256 ≈ 2.5h / 30 epochs (~$6); M4 Pro MPS at batch 256 ≈ 6.5h / 30 epochs ($0, commit `af2bee1`). Choose cheaper target when compute ratio doesn't justify cloud spend.
...
34. **MPS device parity (commit `af2bee1`)**: numpy-sourced tensors created inside training code default to CPU and crash when arithmetic'd with device tensors. All `torch.from_numpy(...)` in `tape/augment.py` and `tape/pretrain.py` must `.to(window.device)` before use. This was a LATENT BUG ON CUDA TOO — prior CPU-only smoke tests hid it; a real H100 run would have crashed at the first augmentation step. Additionally, `torch.linalg.svdvals` is unimplemented on MPS in PyTorch 2.10 → `effective_rank()` falls back to CPU for this once-per-step diagnostic (overhead negligible). Device selection in `scripts/run_pretrain.py` prefers CUDA → MPS → CPU, and `cfg.use_torch_compile` is force-set to False off-CUDA since `mode="reduce-overhead"` requires CUDA graphs.
